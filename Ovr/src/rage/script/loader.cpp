...
namespace rage::ysc {
	...
	void loader::setThread(fnptr<void(program&)> callback) {
		...
		for (uint32_t i{}; i != m_bytecode->m_codePageCollection.codePages.size(); ++i) {
			memcpy(m_program->m_code_blocks[i], m_bytecode->m_codePageCollection.codePages[i], m_bytecode->m_codePageCollection.getCodePageSize(i));
		}
		...
		for (uint32_t i{}; i != m_bytecode->m_stringPageCollection.stringHeaps.size(); ++i) {
			memcpy(const_cast<char*>(m_program->m_string_heaps[i]), m_bytecode->m_stringPageCollection.stringHeaps[i], m_bytecode->m_stringPageCollection.getStringChunkSize(i));
		}
