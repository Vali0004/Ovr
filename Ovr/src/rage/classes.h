...
#pragma pack(pop)
	...
	public:
		scrNativeRegistration* m_entries[256]; //0x0000
	...
	public:
		...
		virtual bool to_json(RsonWriter* jsonStream) { return false; }; //Prints the metric out to a JSON stream
...
public:
	...
	const char* data() {
		std::vector<char> buf{};
		for (uint32_t i{}; i != sizeof(m_data); ++i) {
			if (&m_data[i] && m_data[i]) {
				buf.push_back(m_data[i]);
			}
		}
		return buf.data();
	}
...
namespace rage {
	...
	public:
		...
		//PURPOSE:
		//  Import the data
		static bool ImportTo(class RsonReader* in_msg, class RsonReader* out_event) {
return false;
		}
		static bool IsGamePresEvent(class RsonReader* rr) {
			return false;
		}
		static bool GetEventTypeName(class RsonReader* rr, PresEventName* eventName) {
			return false;
		}
		static bool GetEventData(class RsonReader* rr, RsonReader* out_data) {
			return false;
		}

		//PURPOSE:
		//	Public interface to deserialise that does type checking.
		bool GetObjectAsType(class RsonReader* in_msgData) {
			return false;
		}

		//PURPOSE:
		//	Handles an received event
		virtual void HandleEvent(rlGamerInfo* gamerInfo, class rlScPresenceMessageSender* sender, netGamePresenceEvent& rEvent, class RsonReader* rr, const char* channel) {
			if (Deserialise(rr)) {
				Apply(gamerInfo, nullptr, true);
			}
		}
	...
	protected:
		...
		virtual bool ValidateAuthenticatedHandle(rlGamerHandle* gamerHandle) const {
		...
		virtual bool Serialise(RsonWriter* rw) {
			return true;
		}

		//PURPOSE:
		//   Deserialise the data from json to update this event.  This function will be specific to each event
		virtual bool Deserialise(class RsonReader* /*rw*/) {
			return true;
		}

		//PURPOSE:
		//Apply the data of the event object to carry out the action.
		virtual bool Apply(rlGamerInfo* gamerInfo, rlGamerHandle* gamerHandle, bool isHandleAuthenticated) {
			return true;
		}
	};
}
...
public:
	...
	bool Serialise(rage::RsonWriter* rw) override {
		bool res{ rw->WriteInt("t", m_typeHash) };
		if (m_typeHash == FOG_TYPE_VEHICLE)
...
namespace rage {
	...
	public:
		...
		rlScPresenceMessagePublish(const char* channel, const char* msg) {
			strcpy_s(m_Channel, channel);
